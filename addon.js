...
builder.defineStreamHandler(async a=>{const u=epUrl(a.id);if(!u)return{streams:[]};if(String(a.id).startsWith('animehay:')){try{return{streams:await animeStreams(u),cacheMaxAge:3600}}catch(e){console.error('[anime-stream]',e.message);return{streams:[],cacheMaxAge:0}}}const ep=(u.match(/\/tap-(\d+)/i)||[])[1];if(!ep)return{streams:[]};try{return{streams:await legacyStreams(a.id,ep),cacheMaxAge:86400}}catch(e){console.error('[direct-stream]',e.message);return{streams:[],cacheMaxAge:0}}});
