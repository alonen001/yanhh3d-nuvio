...
function hh3dSearchCards(h){const out=[],seen=new Set();for(const m of String(h||'').matchAll(/<li\b[^>]*>[\s\S]*?<\/li>/gi)){const item=m[0],a=item.match(/<a\b[^>]*href=["']([^"']+)["'][^>]*>/i),label=item.match(/<span\b[^>]*class=["'][^"']*\blabel\b[^"']*["'][^>]*>([\s\S]*?)<\/span>/i);if(!a||!label)continue;const url=abs(a[1],ACTIVE_HH3D_BASE),title=clean(label[1]);if(!url||!title||seen.has(url))continue;seen.add(url);out.push({url,title})}return out}
async function findHh3dMatch(metaId,title){const cached=hh3dMatchCache.get(metaId);if(cached&&Date.now()-cached.time<DOMAIN_CACHE_MS)return cached.item;const simplified=clean(title).replace(/\b(?:3D|2D|Thuyết Minh|Vietsub)\b/gi,' ').replace(/\s+/g,' ').trim(),queries=[title];if(simplified&&simplified!==title)queries.push(simplified);const results=await Promise.allSettled(queries.map(q=>hh3dGet('/wp-json/halim/v1/search?search='+encodeURIComponent(q)))),candidates=[],seen=new Set();for(const result of results){if(result.status!=='fulfilled')continue;for(const item of hh3dSearchCards(result.value)){if(!seen.has(item.url)){seen.add(item.url);candidates.push(item)}}}candidates.sort((a,b)=>titleScore(title,b.title)-titleScore(title,a.title));const best=candidates[0],item=best&&titleScore(title,best.title)>=0.65?best:null;hh3dMatchCache.set(metaId,{time:Date.now(),item});return item}
...
async function hh3dPostId(path){const known={'/tien-nghich':'20224'};if(known[path])return known[path];let last;for(const force of [false,true]){for(const root of await hh3dRoots(force)){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),15000);try{const url=new URL(path,root),response=await fetch(url,{signal:controller.signal,method:'HEAD',redirect:'follow',headers:{'User-Agent':UA,'Accept':'text/html,*/*;q=0.8','Referer':root+'/'}}),link=response.headers.get('link')||'',postId=(link.match(/\/wp\/v2\/posts\/(\d+)/i)||link.match(/[?&]p=(\d+)/i)||[])[1];if(postId){ACTIVE_HH3D_BASE=new URL(response.url||url).origin;return postId}throw Error('HH3D HEAD '+response.status)}catch(e){last=e}finally{clearTimeout(timer)}}}throw last||Error('HH3D post unavailable')}
async function resolveHh3dStream(path,ep){const cleanPath=String(path).replace(/\/$/,'').replace(/\/embed$/i,''),postId=await hh3dPostId(cleanPath),ref=new URL(cleanPath,ACTIVE_HH3D_BASE).href;return hh3dPlayer(postId,ep,ref)}
