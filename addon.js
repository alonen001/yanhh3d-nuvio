...
async function hh3dAlternates(metaId,ep){const cachedTitle=yanTitleCache.get(metaId),slug=detailUrl(metaId).split('/').filter(Boolean).at(-1),title=cachedTitle&&Date.now()-cachedTitle.time<DOMAIN_CACHE_MS?cachedTitle.title:slug.replace(/-/g,' '),match=await findHh3dMatch(metaId,title);if(!match)return[];const hh3dSlug=new URL(match.url).pathname.split('/').filter(Boolean).at(-1);if(!/^[a-z0-9-]+$/i.test(hh3dSlug||''))return[];return[{name:'HH3D Direct',title:'HH3D • 1080 • Vietsub',url:ADDON+'/api/hh3d?slug='+encodeURIComponent(hh3dSlug)+'&ep='+encodeURIComponent(ep),quality:'1080p',behaviorHints:{notWebReady:true}}]}
